...
start thirdparty\llama.cpp\build_blas\bin\Release\server.exe -m models\Qwen-7B-Chat.Q4_K_M.gguf -c 1024 --port 8030 --log-disable --chat-template chatml  &
...
@REM start thirdparty\llama.cpp\build_blas\bin\Release\server.exe -m models\MiniCPM-2B-dpo-q4km-gguf.gguf --port 8050 --log-disable --chat-template chatml &
